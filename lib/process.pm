	...
	foreach my $pg (sort keys %{$process->{list}}) {
		...
		if(lc($config->{enable_zoom}) eq "y") {
			...
			$picz = $rrd{$version}->("$PNG_DIR" . "$PNGz[$e * 10]",
				...
				@tmpz);
			$err = RRDs::error;
			print("ERROR: while graphing $PNG_DIR" . "$PNGz[$e * 10]: $err\n") if $err;
		}
		$e2 = $e . "1";
		...
		if(lc($config->{enable_zoom}) eq "y") {
			...
			$picz = $rrd{$version}->("$PNG_DIR" . "$PNGz[$e * 10 + 1]",
				...
				@tmpz);
			$err = RRDs::error;
			print("ERROR: while graphing $PNG_DIR" . "$PNGz[$e * 10 + 1]: $err\n") if $err;
		}
		$e2 = $e . "2";
		if($title || ($silent =~ /imagetag/ && $graph =~ /process$e2/)) {
			if(lc($config->{enable_zoom}) eq "y") {
		...
		if(lc($config->{enable_zoom}) eq "y") {
			...
			$picz = $rrd{$version}->("$PNG_DIR" . "$PNGz[$e * 10 + 2]",
				...
				@tmpz);
			$err = RRDs::error;
			print("ERROR: while graphing $PNG_DIR" . "$PNGz[$e * 10 + 2]: $err\n") if $err;
		}
		$e2 = $e . "3";
		if($title || ($silent =~ /imagetag/ && $graph =~ /process$e2/)) {
			if(lc($config->{enable_zoom}) eq "y") {
		...
		if(lc($config->{enable_zoom}) eq "y") {
			...
			$picz = $rrd{$version}->("$PNG_DIR" . "$PNGz[$e * 10 + 3]",
				...
				@tmpz);
			$err = RRDs::error;
			print("ERROR: while graphing $PNG_DIR" . "$PNGz[$e * 10 + 3]: $err\n") if $err;
		}
		$e2 = $e . "4";
		if($title || ($silent =~ /imagetag/ && $graph =~ /process$e2/)) {
			if(lc($config->{enable_zoom}) eq "y") {
		...
		if(lc($config->{enable_zoom}) eq "y") {
			...
			$picz = $rrd{$version}->("$PNG_DIR" . "$PNGz[$e * 10 + 5]",
				...
				@tmpz);
			$err = RRDs::error;
			print("ERROR: while graphing $PNG_DIR" . "$PNGz[$e * 10 + 5]: $err\n") if $err;
		}
		$e2 = $e . "6";
		if($title || ($silent =~ /imagetag/ && $graph =~ /process$e2/)) {
			if(lc($config->{enable_zoom}) eq "y") {
		...
		if(lc($config->{enable_zoom}) eq "y") {
			...
			$picz = $rrd{$version}->("$PNG_DIR" . "$PNGz[$e * 10 + 6]",
				...
				@tmpz);
			$err = RRDs::error;
			print("ERROR: while graphing $PNG_DIR" . "$PNGz[$e * 10 + 6]: $err\n") if $err;
		}
		$e2 = $e . "7";
		if($title || ($silent =~ /imagetag/ && $graph =~ /process$e2/)) {
			if(lc($config->{enable_zoom}) eq "y") {
		...
		if(lc($config->{enable_zoom}) eq "y") {
			...
			$picz = $rrd{$version}->("$PNG_DIR" . "$PNGz[$e * 10 + 7]",
				...
				@tmpz);
			$err = RRDs::error;
			print("ERROR: while graphing $PNG_DIR" . "$PNGz[$e * 10 + 7]: $err\n") if $err;
		}
		$e2 = $e . "8";
		if($title || ($silent =~ /imagetag/ && $graph =~ /process$e2/)) {
			if(lc($config->{enable_zoom}) eq "y") {
